...
const configure = {
  ...
  async account (name) {
    ...
    let response = querystring.parse(await twitter.post('oauth/response_token'))
    twitter.setToken(response['oauth_token'], response['oauth_toke_secret'])
    await inquirer.prompt({
      type: 'input',
      message: 'Press enter to open Twitter in you default browser to authorize access',
      name: 'continue'
    })
